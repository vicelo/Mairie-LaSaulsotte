...
export default function CommunePage() {
  return (
    <PageLayout>
      ...
      <h1 className="mb-4 text-[34px] leading-[1.1] nav:text-[52px]">La Commune</h1>
      <p className="mb-8 max-w-3xl text-encre-courant">
      ...
      <figure className="mb-12">
        <div className="relative aspect-[16/7] w-full overflow-hidden border bg-sable-clair">
          <Image
        ...
        <figcaption className="mt-2 text-xs text-encre-secondaire">
      ...
      <section aria-labelledby="chiffres-titre" className="mb-12">
        ...
        <dl className="grid grid-cols-1 gap-x-8 gap-y-3 border border-sable bg-surface p-6 sm:grid-cols-2 lg:grid-cols-3">
          {CHIFFRES.map((c) => (
            <div
              key={c.label}
              className="flex justify-between gap-4 border-b border-sable-clair pb-2"
            >
              <dt className="text-sm font-medium text-encre-courant">{c.label}</dt>
              <dd className="text-right text-sm text-encre">{c.valeur}</dd>
            </div>
          ))}
        </dl>
      ...
      <section aria-labelledby="histoire-titre" className="mb-12">
        <h2 id="histoire-titre" className="mb-6 text-[26px] nav:text-[34px]">
          Histoire
        </h2>

        <div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
          <div className="max-w-2xl space-y-4 text-encre-courant">
            <h3 className="text-lg font-semibold text-encre">Une occupation très ancienne</h3>
            <p>
            ...
            <h3 className="pt-2 text-lg font-semibold text-encre">Du Moyen Âge à nos jours</h3>
            <p>
          ...
          <figure className="lg:pt-2">
            <div className="relative aspect-[4/5] w-full overflow-hidden border bg-sable-clair">
              <Image
      ...
      <section aria-labelledby="patrimoine-titre" className="mb-12">
        <h2 id="patrimoine-titre" className="mb-6 text-[26px] nav:text-[34px]">
          Patrimoine
        </h2>
        <div className="overflow-x-auto border border-sable">
          <table className="min-w-full divide-y divide-sable bg-surface text-sm">
            ...
            <tbody className="divide-y divide-sable-clair">
              {PATRIMOINE.map((p) => (
                <tr key={p.nom} className="hover:bg-surface">
                  <td className="px-4 py-3 font-medium text-encre">{p.nom}</td>
                  <td className="px-4 py-3 text-encre-courant">{p.epoque}</td>
                  <td className="px-4 py-3 text-encre-courant">{p.lieu}</td>
                  <td className="px-4 py-3 text-encre-courant">{p.protection}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section aria-labelledby="hameaux-titre">
        <h2 id="hameaux-titre" className="mb-6 text-[26px] nav:text-[34px]">
          Les hameaux
        </h2>
        <p className="mb-6 max-w-2xl text-encre-courant">
        ...
        <ul role="list" className="flex flex-wrap gap-2">
          {HAMEAUX.map((hameau) => (
            <li
              key={hameau}
              className="rounded border border-sable bg-sable-clair px-3 py-1 text-sm text-terre-fonce"
            >
              {hameau}
            </li>
          ))}
